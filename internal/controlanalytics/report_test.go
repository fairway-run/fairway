...
func TestTaskControlFactPartitionsTriggeredFromPassed(t *testing.T) {
	...
	fact := taskControlFact(task, definition, eval, []store.Review{{Domain: "backend", Verdict: "changes"}, {Domain: "backend", Verdict: "approve"}}, nil)
...
func TestAggregateClassifiesBoundedCohorts(t *testing.T) {
	...
	rows := Aggregate(facts, definitions, thresholds)
	if len(rows) != 2 {
...
func TestAggregateKeepsUnknownCensoredAndUnavailableOutsideOutcomeDenominator(t *testing.T) {
	...
	row := Aggregate(facts, definitions, thresholds)[0]
