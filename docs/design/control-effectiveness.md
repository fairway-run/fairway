...
## Harness Outcome Efficiency

Control effectiveness asks whether a Fairway control is associated with a
better engineering outcome. Harness outcome efficiency asks a narrower,
task-local question: how many recorded execution attempts, material actions,
tokens, and elapsed seconds were used per evaluator-backed outcome?

The versioned `fairway.harness-analysis.v1` projection is generated by:

```bash
fairway harness report --task <task-id> --format json
```

It joins only task-scoped, source-qualified external-run, observation,
evaluator-result, and provider-usage records. A result of `pass`, `fail`,
`partial`, or `inconclusive` counts as evaluator-backed because an evaluator
actually produced a judgment; `error` and `unavailable` do not. This count is
not a success rate. Attempts/actions per outcome and tokens/elapsed time per
outcome are withheld when there is no outcome or the corresponding denominator
is incomplete. Cost remains explicitly unavailable until Fairway has a
comparable exact cost record rather than a price inference.

The first implementation emits one named compatibility cohort per task report.
It withholds every efficiency ratio when the task contains multiple run-source,
provider/model/harness, action-source/completeness,
evaluator/source/subject identity/environment, or usage-source/confidence
dimensions. Attempts,
recorded actions, and evaluator results must also carry external-run
attribution; provider usage contributes token or elapsed denominators only when
every usage event names a session referenced by that run cohort. Estimated or
mixed usage remains visibly qualified rather than presented as exact. This is
deliberately conservative: a later report can expose several separately named
cohorts, but it must not collapse them into one ratio.

The same projection may identify repeated failed/inconclusive evaluations,
repeated rejected/inconclusive action fingerprints, or an active checkpoint
with no new durable observation, evaluator result, evidence, or decision for a
bounded period. Every finding cites its source facts and states a false-positive
limit. Findings are advisory: they cannot contact a provider, redirect work,
change task state, accept evidence, create or satisfy a review, alter policy, or
authorize promotion. They are not used to rank tasks, people, models, or
providers across incompatible evaluators or environments.
